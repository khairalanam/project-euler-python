...
def find_digit_power_sums(power):
    max_value = (9**power) * len(str(9**power))
    result = sum([num for num in range(10, max_value + 1)
                 if num == sum_of_digits_powered(num, power)])

    return result


print(find_digit_power_sums(5))
